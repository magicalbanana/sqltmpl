...
// Right now, each test needs to be hand-defined with its own struct and test
// method.
...
func TestStructParameters(test *testing.T) {
	...
	//
	prsr = NewParser("SELECT * FROM table WHERE col1 = :Foo AND col2 = :Bar AND col3 = :Baz")
	prsr.SetValuesFromStruct(singleParam)

	verifyStructParameters("MultipleStructReplacement", test, prsr, []interface{}{
		"foo",
		"bar",
		15,
	})
	...
	//
	prsr = NewParser("SELECT * FROM table WHERE col1 = :Foo AND col2 = :Bar AND col3 = :Foo AND col4 = :Foo AND col5 = :Baz")
	prsr.SetValuesFromStruct(singleParam)

	verifyStructParameters("RecurringStructParameterReplacement", test, prsr, []interface{}{
		"foo",
		"bar",
		"foo",
		"foo",
	...
	//
	prsr = NewParser("SELECT * FROM table WHERE col1 = :unexported AND col2 = :notExported AND col3 = :Foo")
	prsr.SetValuesFromStruct(singleParam)

	verifyStructParameters("UnexportedStructReplacement", test, prsr, []interface{}{
		nil,
		nil,
		"foo",
	})
...
func verifyStructParameters(testName string, test *testing.T, prsr Parser, expectedParameters []interface{}) {

	var actualParameters []interface{}

	actualParameters = prsr.GetParsedParameters()
	...
	if actualParameterLength != expectedParameterLength {
		test.Log("Test ", testName, ": Actual parameters (", actualParameterLength, ") returned from struct prsr did not match expected parameters (", expectedParameterLength, ")")
		test.Fail()
	}
